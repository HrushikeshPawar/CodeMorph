CREATE OR REPLACE PACKAGE BODY invoice_pkg AS
  ...
  PROCEDURE create_invoice_for_project (
    ...
    v_project_details schema_app_core.projects%ROWTYPE; -- Test: Inter-schema %ROWTYPE usage
    v_invoice_id NUMBER;
  BEGIN
    v_project_details := schema_app_core.project_pkg.get_project_details(p_project_id => p_project_id); -- Test: Call to another schema's package

    IF v_project_details.id IS NULL THEN
      schema_util_common.logger_pkg.log_error('Project not found for invoicing: ' || p_project_id);
      RETURN;
    END IF;
    ...
    schema_util_common.logger_pkg.log_message(3, 'Invoice ' || v_invoice_id || ' created for project ' || v_project_details.name);
  ...
  BEGIN
    schema_util_common.logger_pkg.log_debug('Attempting to send invoice: ' || p_invoice_id);
    ...
    END IF;
    schema_util_common.logger_pkg.log_info('Invoice ' || p_invoice_id || ' marked as sent.');
  EXCEPTION
    WHEN OTHERS THEN
       schema_util_common.logger_pkg.log_error('Failed to send invoice: ' || p_invoice_id);
