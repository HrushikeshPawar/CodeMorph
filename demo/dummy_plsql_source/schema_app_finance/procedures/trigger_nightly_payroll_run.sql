...
BEGIN
  util_common.logger_pkg.log_info('Starting nightly payroll run.');
  ...
  FOR emp_rec IN c_all_employees LOOP
    ...
    EXCEPTION
      WHEN OTHERS THEN
        util_common.logger_pkg.log_error('Failed payroll for emp: ' || emp_rec.id);
    END;
  END LOOP;

  util_common.logger_pkg.log_info('Nightly payroll run completed.');
