...
BEGIN
  schema_util_common.logger_pkg.log_info('Archiving projects older than: ' || schema_util_common.date_utils_pkg.format_date(p_cutoff_date)); -- Test: Call to schema.pkg.func
  ...
  LOOP
    ...
    schema_util_common.logger_pkg.log_debug('Archiving project: ' || v_project_rec.name);
...
EXCEPTION
  WHEN OTHERS THEN
    schema_util_common.logger_pkg.log_error('Error archiving projects.');
