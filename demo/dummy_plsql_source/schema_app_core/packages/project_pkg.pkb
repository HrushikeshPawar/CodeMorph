CREATE OR REPLACE PACKAGE BODY project_pkg AS
  ...
  BEGIN
    schema_util_common.logger_pkg.log_info('Creating project: ' || p_name);
    ...
    IF v_lead_employee.id IS NULL THEN
      schema_util_common.logger_pkg.log_error('Invalid lead employee ID: ' || p_lead_emp_id);
  ...
  ) RETURN schema_app_core.projects%ROWTYPE IS
    v_project_rec schema_app_core.projects%ROWTYPE;
  BEGIN
    -- Simulate SELECT * INTO v_project_rec FROM schema_app_core.projects WHERE id = p_project_id;
  ...
  BEGIN
    ...
    schema_util_common.logger_pkg.log_debug('Assigned '||p_emp_id||' to project '||p_project_id||' as '||p_role);
