...
- Performance optimization: Precompiled literal restoration regex in `CallDetailExtractor` to avoid repeated compilation in parameter restoration loops, resulting in ~32% speed improvement for that component.
- Performance optimization: Removed redundant regex evaluation in `PlSqlStructuralParser` for `END` keyword checks.
